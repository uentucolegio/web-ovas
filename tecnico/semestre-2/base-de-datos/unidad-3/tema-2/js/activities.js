...
function setupSelection() {
    ...
    function updateSelection(cell) {
        if (!cell || !startCell || cell === startCell) return;
    ...
    container.addEventListener('pointermove', e => {
        ...
        if (cell && cell !== startCell) draggedSelection = true;
        updateSelection(cell);
        e.preventDefault();
    });

    container.addEventListener('pointerup', finishSelection);
    container.addEventListener('pointercancel', finishSelection);

    container.addEventListener('click', e => {
        const cell = e.target.closest('.grid-cell');
        if (!cell || selecting) return;
        if (draggedSelection) {
            draggedSelection = false;
            return;
        }
        ...
        const r1 = +clickStartCell.dataset.row, c1 = +clickStartCell.dataset.col;
        const r2 = +cell.dataset.row, c2 = +cell.dataset.col;
        const cells = getCellsInLine(r1, c1, r2, c2);
        clearHighlight();
        clickStartCell = null;
        if (cells.length >= 2) checkSelection(cells);
    });
}
