// Interactividad del Tema 3: progreso, sopa de letras, asociación y modal.
...
const activityConfig = {
    ...
    pairsPerRound: 6,
    words: [
        'INTEGRIDAD', 'REFERENCIAL', 'PRIMARIA', 'FORANEA', 'CASCADE', 'RESTRICT',
        'SETNULL', 'SETDEFAULT', 'HUERFANO', 'RELACION', 'REFERENCIADA',
        'REFERENCIANTE', 'INSERCION', 'ELIMINACION', 'ACTUALIZACION',
        'CONSISTENCIA', 'RESTRICCION', 'CLAVE', 'REGISTRO', 'TABLA',
        'DEPENDENCIA', 'VINCULO'
    ],
    pairs: [
        ['Integridad referencial', 'Garantiza que toda clave foránea apunte a una clave válida de la tabla relacionada.'],
        ['Clave primaria (PK)', 'Identifica de manera única cada registro de una tabla.'],
        ['Clave foránea (FK)', 'Atributo que referencia la clave primaria de otra tabla.'],
        ['Registro huérfano', 'Registro hijo cuya referencia no corresponde a ningún registro padre.'],
        ['CASCADE', 'Propaga al registro hijo la actualización o eliminación realizada en el padre.'],
        ['RESTRICT', 'Impide modificar o eliminar un registro padre mientras tenga registros relacionados.'],
        ['SET NULL', 'Conserva el registro hijo y cambia su clave foránea a un valor nulo.'],
        ['SET DEFAULT', 'Asigna a la clave foránea un valor predeterminado cuando cambia o se elimina el padre.'],
        ['Tabla referenciada', 'Tabla padre que contiene la clave primaria utilizada por otra tabla.'],
        ['Tabla referenciante', 'Tabla hija que contiene la clave foránea.'],
        ['Consistencia', 'Estado en el que las relaciones entre los datos cumplen las reglas definidas.'],
        ['Restricción', 'Regla aplicada por el SGBD para evitar datos inválidos.']
    ]
};
...
function updateProgress(type) {
    ...
    if (type === 'content') {
        completed.forEach(id => {
            ...
            if (badge) badge.className = 'px-3 py-1 rounded-full bg-green-100 text-green-800 border border-green-400';
        });
    }
...
document.querySelectorAll('.challenge-option').forEach(button => {
    button.addEventListener('click', () => {
        ...
        markAnswer(button, isCorrect, document.getElementById(`feedback-${id}`), '🎉 ¡Correcto! Ganaste 25 XP.', '💪 Casi. Revisa el caso e inténtalo otra vez.');
        if (isCorrect) {
...
function checkSelection() {
    ...
    if (match) {
        ...
        setWordFeedback(finished ? `🏆 ¡Excelente! Encontraste las ${wordSearch.placed.length} palabras. +25 XP.` : `✅ Encontraste ${match.word}. Continúa con las demás.`, true);
...
function evaluateMatch() {
    ...
    if (term.dataset.id === definition.dataset.id) {
        ...
        setMatchingFeedback(finished ? `🏆 ¡Excelente! Relacionaste los ${matching.pairs.length} conceptos. +25 XP.` : '✅ Relación correcta. Continúa con el siguiente par.', true);
